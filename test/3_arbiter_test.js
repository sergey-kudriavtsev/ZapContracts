...
contract('Arbiter', function (accounts) {
    ...
    it("ARBITER_2 - initiateSubscription() - Check subscription block must be more than 0", async function () {
        ...
        await expect(this.test.arbiter.initiateSubscription(oracle, params, specifier, publicKey, 0, {from: subscriber})).to.eventually.be.rejectedWith(EVMRevert);
    });
    ...
    it("ARBITER_3 - initiateSubscription() - Check user can inititate subscription for same subscriber once", async function () {
        ...
        await expect(this.test.arbiter.initiateSubscription(oracle, params, specifier, publicKey, 5, {from: subscriber})).to.eventually.be.rejectedWith(EVMRevert);
    });
